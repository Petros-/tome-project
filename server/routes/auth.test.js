    ...
    describe("before signup", () => {
      describe("POST /", () => {
        it("should return 401, because you can't get in if you haven't signed up", async () => {
          const res = await request(serverInstance).post("/auth/login").send(user0);
      ...
      describe("PUT /password", () => {
        it("should return 401", async () => {
          const res = await request(serverInstance).put("/auth/password").send(user0);
          expect(res.statusCode).toEqual(401);
        });
      });
      ...
      describe("POST /logout", () => {
        it("should return 404", async () => {
          const res = await request(serverInstance).post("/auth/logout").send();
    ...
    describe("signup ", () => {
      describe("POST /signup", () => {
        it("should return 400 if no password is provided", async () => {
          const res = await request(serverInstance).post("/auth/signup").send({
            email: user0.email,
          });
        ...
        it("should return 200 if signup is successful and account is created", async () => {
          const res = await request(serverInstance).post("/auth/signup").send(user1);
          expect(res.statusCode).toEqual(200);
        });
  
        it("should return 409 Conflict with a repeat signup", async () => {
          let res = await request(serverInstance).post("/auth/signup").send(user0);
          expect(res.statusCode).toEqual(200);
          res = await request(serverInstance).post("/auth/signup").send(user0);
    ...
    describe.each([user0, user1])("User %#", (user) => {
      ...
      describe("POST /", () => {
        it("should return 400 when a password isn't provided", async () => {
          const res = await request(serverInstance).post("/auth/login").send({
            email: user.email,
          });
          expect(res.statusCode).toEqual(400);
        });
        ...
        it("should return 200 and a token when the correct password is provided", async () => {
          const res = await request(serverInstance).post("/auth/login").send(user);
          expect(res.statusCode).toEqual(200);
      ...
      describe("PUT /password", () => {
        ...
        it("should change the password for user0", async () => {
          const res = await request(serverInstance)
            ...
            .send({ password: "123" });
          expect(res.statusCode).toEqual(200);
          let loginRes0 = await request(serverInstance).post("/auth/login").send(user0);
          expect(loginRes0.statusCode).toEqual(401);
          loginRes0 = await request(serverInstance).post("/auth/login").send({
            email: user0.email,
            password: "123",
          });
          expect(loginRes0.statusCode).toEqual(200);
          const loginRes1 = await request(serverInstance).post("/auth/login").send(user1);
        ...
        it("should change the password for user1", async () => {
          const res = await request(serverInstance)
            ...
            .send({ password: "123" });
          expect(res.statusCode).toEqual(200);
          const loginRes0 = await request(serverInstance).post("/auth/login").send(user0);
          expect(loginRes0.statusCode).toEqual(200);
          let loginRes1 = await request(serverInstance).post("/auth/login").send(user1);
          expect(loginRes1.statusCode).toEqual(401);
          loginRes1 = await request(serverInstance).post("/auth/login").send({
            email: user1.email,
            password: "123",
          });
          expect(loginRes1.statusCode).toEqual(200);
